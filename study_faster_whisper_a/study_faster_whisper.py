        ...
        if silent_chunks > (SILENCE_DURATION * RATE / CHUNK):
            if recording:
                if len(frames) * CHUNK / RATE < MIN_AUDIO_LENGTH:
                    ...
                    file_path = (
                        audio_directory / f"recorded_audio_{file_name}_latest.wav"
                    )
            ...
            if speak_chunks > (OUT_DURATION * RATE / CHUNK):
                file_path = (
                    audio_directory / f"recorded_audio_{file_name}_{speak_cnt}.wav"
                )
                executor.submit(save_wave_file, Path(file_path), frames)
                speak_cnt += 1
                speak_chunks = 0
...
class FileHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        if not file_name.endswith("_latest"):
            ...
            if os.path.exists(
                os.path.join(os.path.dirname(event.src_path), f"{base_name}_latest.wav")
            ):
            ...
            if os.path.exists(
                os.path.join(
                    os.path.dirname(event.src_path), f"{base_name}_{suffix + 1}.wav"
                )
            ):
        ...
        try:
            with open(file_path, "rb") as audio_file:
                segments, _ = model.transcribe(
                    audio_file, language="ja", beam_size=5, patience=0.5
                )
